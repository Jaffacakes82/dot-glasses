...
public class AuditSaveChangesInterceptorTests(PostgresContainerFixture postgres)
{
    private static DotGlassesDbContext CreateContext(string connectionString, FakeCurrentUserContext currentUserContext)
    {
        ...
        return PostgresContainerFixture.CreateContext(
            connectionString,
            httpContextAccessor,
            new AuditSaveChangesInterceptor(currentUserContext));
    }
    ...
    public async Task Adding_PopulatesCreatedFields_NotModifiedFields()
    {
        ...
        await using var context = CreateContext(await postgres.CreateDatabaseAsync(), currentUser);
    ...
    public async Task Modifying_PopulatesModifiedFields_LeavesCreatedFieldsIntact()
    {
        ...
        await using (var createContext = CreateContext(connectionString, creator))
        {
        ...
        await using var editContext = CreateContext(connectionString, editor);
    ...
    public async Task Removing_IsRewrittenAsSoftDelete_RowStillPresentButFlagged()
    {
        ...
        await using (var createContext = CreateContext(connectionString, currentUser))
        {
            createContext.WidgetExamples.Add(new WidgetExample { Id = widgetId, Name = "Test", HierarchyPath = "/1/" });
            await createContext.SaveChangesAsync();
        }

        await using (var deleteContext = CreateContext(connectionString, currentUser))
        {
        ...
        await using var verifyContext = CreateContext(connectionString, currentUser);
